...
Search.prototype.init = function init() {
  var viewCount = js_cookie.get('searchresultVisits') ? parseInt(js_cookie.get('searchresultVisits'), 10) : 0;
  ...
  window.location.search && js_cookie.set('searchresultVisits', ++viewCount, {
    expires: 2 / 1440
  });
  ...
  if (this._recaptchaRequired && !this._recaptchaVerified && window.location.search) {
    ...
    for (var i$1 = this._inputs.length - 1; i$1 >= 0; i$1--) {
      this._inputs[i$1].name = "s";
    }
  }
};
/**
...
Search.prototype._resetCookie = function _resetCookie() {
  js_cookie.set('searchresultVisits', 0);
};

Search.prototype._addNotification = function _addNotification(remove) {
  var info = document.createElement("p");
  info.classList.add("mb-3");
  info.innerHTML = "Please verify you are Human.";
  var recaptchaEl = document.getElementById('search-recaptcha');
...
Search.prototype._initRecaptcha = function _initRecaptcha() {
  ...
  $script.attr('src', 'https://www.google.com/recaptcha/api.js' + '?onload=searchCallback&render=explicit').prop({
    async: true,
    defer: true
  });
